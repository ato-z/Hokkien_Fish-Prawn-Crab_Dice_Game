...
export const EditRoomPage = () => {
  ...
  // 验证房主身份并加载房间数据
  useEffect(() => {
    const verifyOwnerAndLoadRoom = async () => {
      // TODO: 这里应该调用 API 验证用户是否是房主
      // 暂时使用 localStorage 模拟
      const mockRooms: Room[] = MOCK_ROOMS
      ...
      // TODO: 这里应该验证当前用户是否是房主
      // 暂时假设 room.ownerId 存在并与 currentUserId 比较
      const isOwner = true // 模拟当前用户是房主
  ...
  const handleSubmit = (e: React.ChangeEvent<HTMLFormElement>) => {
    ...
    const updatedRoom: Room = {
      ...roomData,
      name: formData.name || roomData.name,
      sub: formData.sub || roomData.sub,
      minBet: formData.minBet ?? roomData.minBet,
      jetton: formData.jetton || roomData.jetton,
      maxPlayers: formData.maxPlayers || roomData.maxPlayers,
      tag: currentTag || roomData.tag,
      private: formData.private,
      password: formData.password,
    }

    console.log('更新房间:', updatedRoom)

    // TODO: 调用 API 更新房间
    // 暂时使用 localStorage 模拟
    const mockRooms: Room[] = JSON.parse(localStorage.getItem('rooms') || '[]')
    const updatedRooms = mockRooms.map((r) => (r.id === roomId ? updatedRoom : r))
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen w-full relative font-sans text-slate-200">
        <BackgroundCommon />
        <div className="flex items-center justify-center min-h-screen">
          <div className="text-slate-400">验证房主身份中...</div>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="min-h-screen w-full relative font-sans text-slate-200">
        ...
        <div className="flex flex-col items-center justify-center min-h-screen gap-4">
          ...
          <div className="text-slate-400">只有房主才能编辑房间</div>
          <button
            type="button"
  ...
  return (
    <div className="min-h-screen w-full relative font-sans text-slate-200 pb-12">
      ...
      <header className="sticky top-0 z-50 bg-slate-950/80 backdrop-blur-md border-b border-white/5 px-4 py-3 flex items-center gap-4">
        ...
        <h1 className="text-lg font-bold text-slate-100">编辑盘口 (Edit Room)</h1>
      </header>
              ...
              <div className="relative z-10 flex justify-between items-start">
                <div className="space-y-1 flex-1 min-w-0 pr-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="px-1.5 py-0.5 rounded-lg bg-amber-950/40 text-amber-400 text-[9px] border border-amber-900/30 font-bold">
                      EDIT
                    </span>
                  </div>
