...
export const GameBoard = ({ gameController, onChoiceTap }: GameBoardProps) => {
    ...
    <aside className="aspect-square w-full game-container flex flex-col gap-0.5">
      ...
      <section className="h-1/5 w-full flex gap-0.5">
        <div className="w-1/5 bg-body">
          <ul className="flex flex-col" onClick={() => onChoiceTap('set', GAME_TAGS.set[0])}>
        ...
        <div className="w-1/5 bg-body">
          <ul className="flex flex-col" onClick={() => onChoiceTap('set', GAME_TAGS.set[1])}>
      ...
      <section>
        <div className="h-full grid grid-cols-6 gap-0.5 text-black">
          {GAME_TAGS.single.map((i) => (
            <div className="size-full" key={i} onClick={() => onChoiceTap('single', i)}>
